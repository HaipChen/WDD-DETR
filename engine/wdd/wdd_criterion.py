...
@register()
class WDDCriterion(nn.Module):
    """ This class computes the loss for WDD.
    """
    ...
    def loss_labels_mal(self, outputs, targets, indices, num_boxes, values=None):
        ...
        # print(" ### WDD-gamma{}-alpha{} ### ".format(self.gamma, self.mal_alpha))
        loss = F.binary_cross_entropy_with_logits(src_logits, target_score, weight=weight, reduction='none')
        loss = loss.mean(1).sum() * src_logits.shape[1] / num_boxes
        return {'loss_mal': loss}

    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        """Compute the losses related to the bounding boxes, the L1 regression loss, the GIoU loss,
           and the NWD (Normalized Wasserstein Distance) loss for tiny objects.
        ...
        # ==================== NWD Loss ====================
        cx_p, cy_p, w_p, h_p = src_boxes.chunk(4, dim=-1)
        cx_t, cy_t, w_t, h_t = target_boxes.chunk(4, dim=-1)

        center_distance = (cx_p - cx_t) ** 2 + (cy_p - cy_t) ** 2
        wh_distance = ((w_p - w_t) ** 2 + (h_p - h_t) ** 2) / 4.0

        w2_squared = center_distance + wh_distance

        nwd = torch.exp(-torch.sqrt(w2_squared + 1e-7) / 12.8)
        loss_nwd = 1 - nwd

        loss_nwd = loss_nwd if boxes_weight is None else loss_nwd * boxes_weight.unsqueeze(-1)
