...
from .wdd_utils import MishFFN, ZISGate, RMSNorm, MLP

__all__ = ['WDDTransformer']
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        if use_gateway:
            self.gateway = ZISGate(d_model, use_rmsnorm=True)
        else:
            self.norm2 = RMSNorm(d_model)

        # ffn
        self.swish_ffn = MishFFN(d_model, dim_feedforward // 2, d_model)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, hidden_dim, decoder_layer, decoder_layer_wide, num_layers, num_head, reg_max, reg_scale, up,
                 eval_idx=-1, layer_scale=2, act='mish'):
...
@register()
class WDDTransformer(nn.Module):
